...
const AP_Param::GroupInfo AP_SerialManager::var_info[] = {
    ...
    // @Values: -1:None, 1:GCS Mavlink, 3:Frsky D-PORT, 4:Frsky S-PORT, 5:GPS, 7:Alexmos Gimbal Serial, 8:SToRM32 Gimbal Serial, 9:Lidar
    // @User: Standard
    AP_GROUPINFO("2_PROTOCOL",  3, AP_SerialManager, state[2].protocol, SerialProtocol_NAV),
    ...
    // @Values: 1:1200,2:2400,4:4800,9:9600,19:19200,38:38400,57:57600,111:111100,115:115200,500:500000,921:921600,1500:1500000
    // @User: Standard
    AP_GROUPINFO("2_BAUD", 4, AP_SerialManager, state[2].baud, AP_SERIALMANAGER_NAV_BAUD/1000),
    ...
    // @Values: -1:None, 1:GCS Mavlink, 3:Frsky D-PORT, 4:Frsky S-PORT, 5:GPS, 7:Alexmos Gimbal Serial, 8:SToRM32 Gimbal Serial, 9:Lidar
    // @User: Standard
    AP_GROUPINFO("4_PROTOCOL",  7, AP_SerialManager, state[4].protocol, SerialProtocol_DEBUG),
    ...
    // @Values: 1:1200,2:2400,4:4800,9:9600,19:19200,38:38400,57:57600,111:111100,115:115200,500:500000,921:921600,1500:1500000
    // @User: Standard
    AP_GROUPINFO("4_BAUD", 8, AP_SerialManager, state[4].baud, AP_SERIALMANAGER_DEBUG_BAUD/1000),
    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        if (state[i].uart != NULL) {
            switch (state[i].protocol) {
				...
				case SerialProtocol_NAV:
					// Note baudrate is hardcoded to 115200
					state[i].baud = AP_SERIALMANAGER_NAV_BAUD / 1000;
					state[i].uart->begin(AP_SERIALMANAGER_NAV_BAUD,
										 AP_SERIALMANAGER_NAV_BUFSIZE_RX,
										 AP_SERIALMANAGER_NAV_BUFSIZE_TX);
					break;

				case SerialProtocol_DEBUG:
					// Baud -- 115200
					state[i].baud = AP_SERIALMANAGER_DEBUG_BAUD / 1000;
					state[i].uart->begin(AP_SERIALMANAGER_DEBUG_BAUD,
										 AP_SERIALMANAGER_DEBUG_BUFSIZE_RX,
										 AP_SERIALMANAGER_DEBUG_BUFSIZE_TX);
					break;
            }
        }
    }
